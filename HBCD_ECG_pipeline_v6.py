...
def parse_command_line_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["participant_level"],
        help="Analysis level. Only participant_level is currently supported.",
    )
    parser.add_argument(
        "--participant-label",
        nargs="+",
        default=None,
        help="Participant labels to process, e.g. 100247 100248.",
    )
    parser.add_argument(
        "--session-label",
        nargs="+",
        default=None,
        help="Session labels to process, e.g. V03 V04.",
    )
    parser.add_argument(
        "--tasks",
        nargs="+",
        default=["RS"],
        help="Task labels to process, e.g. RS MMN FACE VEP.",
    )
    parser.add_argument(
        "--acq",
        default="ecg",
        help="Acquisition label used to match acq-* files.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="ECG quality cutoff for high-quality segment detection.",
    )
    parser.add_argument(
        "--qc-window-sec",
        type=float,
        default=10.0,
        help="Seconds before/after event markers for QC plots.",
    )

    args = parser.parse_args()

    input_path = Path(args.bids_dir).expanduser().resolve()
    output_path = Path(args.output_dir).expanduser().resolve()

    if not input_path.exists():
        parser.error(f"Input directory does not exist: {input_path}")
    if not input_path.is_dir():
        parser.error(f"Input path is not a directory: {input_path}")
    if input_path == output_path:
        parser.error("Input and output directories must be different.")

    if not args.tasks:
        parser.error("At least one task must be supplied.")
    if any(not str(task).strip() for task in args.tasks):
        parser.error("Task labels cannot be empty.")

    if args.participant_label:
        if any("/" in str(label) or "\\" in str(label) for label in args.participant_label):
            parser.error("Participant labels must not contain path separators.")

    if args.session_label:
        if any("/" in str(label) or "\\" in str(label) for label in args.session_label):
            parser.error("Session labels must not contain path separators.")

    if not args.acq.strip():
        parser.error("--acq cannot be empty.")

    if not 0.0 <= args.quality_threshold <= 1.0:
        parser.error("--quality-threshold must be between 0 and 1.")

    if args.qc_window_sec <= 0:
        parser.error("--qc-window-sec must be greater than zero.")
    ...
    return argparse.Namespace(
        ...
        tasks=tuple(str(task).strip().upper() for task in args.tasks),
        acq=args.acq.strip(),
...
class PipelineConfig:
    """Validated runtime configuration for the ECG pipeline."""

    input_dir: Path
    output_dir: Path
    tasks: tuple[str, ...]
    acq: str
    participant_labels: tuple[str, ...] | None
    session_labels: tuple[str, ...] | None
...
def process_file(
    ...
    """Process one EEG .set file and return whether processing succeeded."""
...
def main() -> int:
    ...
    config = PipelineConfig(
        ...
        acq=args.acq,
        participant_labels=(
            tuple(args.participant_label)
            if args.participant_label
            else None
        ),
        session_labels=(
            tuple(args.session_label)
            if args.session_label
            else None
        ),
        quality_threshold=args.quality_threshold,
    ...
    for index, file_path in enumerate(file_paths, start=1):
        if not process_file(file_path, index, total_files, config):
            failed_files += 1

    print("\nPipeline finished.")
    print(f"Base output folder: {config.output_dir}")
